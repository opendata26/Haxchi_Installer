...
with open('Your_Wii_U_IP.txt', 'r') as ipinput:
    servrip = ipinput.read()

class wupclient:
    s=None

    def __init__(self, ip=servrip, port=1337):
    ...
    def chmod(self, filename, flags):
        ...
        ret = self.FSA_ChangeMode(fsa_handle, filename, flags)
    ...
    def up(self, local_filename, filename = None):
        ...
        while True:
            ...
            if len(data) < block_size:
                break
        ret = self.FSA_CloseFile(fsa_handle, file_handle)
        print(local_filename + " replaced on Wii U.")
